...
middlewareObj.checknationalParkOwenership = function(req, res, next) {
  if (req.isAuthenticated()) {
    nationalPark.findById(req.params.id, (err, foundnationalPark) => {
      if (err || !foundnationalPark) {
        ...
        // does the user own the nationalPark
        if (foundnationalPark.author.id.equals(req.user._id) || req.user.isAdmin) { next(); }
        else {
